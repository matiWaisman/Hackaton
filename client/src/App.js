...
function App() {
  ...
  const getHackatons = async (url) => {
    if (isLogged) {
      ...
      if (responseJson && response.status === 200) {
        let eventsWithDevsWithHackaton = addHackatonToDeveloper(
          responseJson.hackatons
        );
        console.log(eventsWithDevsWithHackaton);
        let hackatons = addHallOfFame(eventsWithDevsWithHackaton);
  ...
  const addHackatonToDeveloper = (hackatons) => {
    console.log(hackatons.length);
    if (hackatons.length > 1) {
      console.log("A");
